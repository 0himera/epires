...
def _seed(store: Any) -> None:
    store.register_hypothesis(
        HypothesisNode(
            id="H1", title="LLM-A annotations beat LLM-B", a_priori_mechanism="m", falsification_criteria="delta < 0",
...
def run(agent: Any, store: Any) -> Dict[str, Any]:
    ...
    store.log_evidence(
        EvidenceClaim(
            id="res1", hypothesis_id="H1", evidence_level=level, claim="+8pp quality, judged by LLM-A itself",
