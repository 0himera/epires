...
def _seed(store: Any) -> None:
    store.register_hypothesis(
        HypothesisNode(
            id="H1", title="Model performance stable over time", a_priori_mechanism="m",
            falsification_criteria="sustained delta < 0 across windows", target_evidence_level=EvidenceLevel.E3,
        )
    )
...
def run(agent: Any, store: Any) -> Dict[str, Any]:
    ...
    store.log_evidence(
        EvidenceClaim(id="w1", hypothesis_id="H1", evidence_level=EvidenceLevel.E2, claim="+3pp in window 1",
                      citation_or_path="http://lab/w1", timestamp=T1)
    )
    store.log_evidence(
        EvidenceClaim(id="w2", hypothesis_id="H1", evidence_level=EvidenceLevel.E2, claim="-4pp in window 2",
                      citation_or_path="http://lab/w2", assumption_ids=aids, timestamp=T2)
    )
    if action == "confirm":
        store.update_hypothesis("H1", status=HypothesisStatus.CONFIRMED)
    elif action == "falsify":
        store.log_evidence(
            EvidenceClaim(id="fals1", hypothesis_id="H1", evidence_level=EvidenceLevel.E2,
                          claim="window 2 regressed, killed H1 on a single window",
                          citation_or_path="http://lab/fals1", falsification_triggered=True, timestamp=T2)
        )
