...
# Диагностический доктор: проверка целостности БД, MCP инструментов и конфигов
epires doctor

# Вывод канонической JSON-схемы и Python-сниппета для миграции заметок
epires schema [--format json|python]

# Массовый импорт исторических гипотез из Markdown, JSON или JSONL
epires ingest [findings.md] [--dry-run] [--upsert]

# Генерация шаблона скрипта миграции под кастомный формат заметок
epires ingest --template scripts/migrate_findings.py

# Экспорт графа знаний в переносимый JSON-бандл с контрольной суммой SHA256 (для Git/CI)
epires export [--out research-graph.json] [--format json|jsonl]

# Восстановление графа знаний из JSON-бандла
...
| `epires_update_hypothesis` | Обновление статуса (`PROPOSED`, `IN_PROGRESS`, `CONFIRMED`, `FALSIFIED`, `BLOCKED`, `REFINED`), целевого уровня, родителей DAG, сущностей, тегов или текста |
...
| `epires_list_relations` | Выгрузка сохраненных связей графа с фильтрацией по типу отношения |
| `epires_bulk_import` | Транзакционный импорт пачки гипотез и свидетельств из JSON |
...
| `audit_hypothesis` | Детерминированный аудит provenance, шлюзов и целостности ledger одной гипотезы |
| `algedonic_check` | Поиск противоречий, провалов аудита, массовых блокировок и исчерпания бюджета трейсов |
| `algedonic_freeze` | Карантин ветви с каскадным статусом `BLOCKED` для зависимых потомков |
| `score_experiments` | Ранжирование кандидатов экспериментов по заданному вектору весов качества |
| `calibrated_p` | Коррекция заявленной агентом вероятности по истории калибровки |
| `pheromone_rank` | Ранжирование гипотез по стигмергическому весу активности |
| `compute_evidence_level` | Пересчёт агрегированного уровня по выбранным ID свидетельств и шлюзам |
| `s3_audit_confirmed` | Независимый S3*-аудит всех гипотез со статусом `CONFIRMED` |
...
* **Retraction & Unblock Invariant**: отзыв фальсифицирующего свидетельства строго восстанавливает статус и разблокирует всех потомков, чьи родители снова валидны.
