...
def test_cli_atomic_register_and_log_evidence(monkeypatch, tmp_path):
...
def test_cli_dag_frontier_filter(tmp_path):
    init_workspace(target_dir=tmp_path)
    store = EpiresStore(db_path=tmp_path / ".epires" / "hypotheses.db")
...
def test_doctor_redundancy_check(tmp_path):
...
def test_cli_scaffold_and_verify_gates(monkeypatch, tmp_path):
    init_workspace(target_dir=tmp_path)
    store = EpiresStore(db_path=tmp_path / ".epires" / "hypotheses.db")
